...
# The database we will use are a group  of 100 children for which a number of measurements have been carried out in the field 
...
child.data=read.table('data.txt', head=T)

# Transform character variables into factors 
child.data$sexe=factor(child.data$sexe)
child.data$sickGIparasites=factor(child.data$sickGIparasites)
child.data$malaria=factor(child.data$malaria)
...
m3=lm(taille~age+sexe, data=child.data)
summary(m3)
...
m4=lm(taille~age+sexe+GIparasites+malaria, data=child.data)
summary(m4)
...
m6=glm(GIparasites~age+sexe+malaria, family='poisson', data=child.data)
summary(m6)
...
m8=glm(sickGIparasites~age+sexe++malaria, family='binomial', data=child.data)
summary(m8)
