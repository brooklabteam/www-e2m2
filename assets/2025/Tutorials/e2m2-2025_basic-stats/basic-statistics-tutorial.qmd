---
title: "E2M2 2025: Basic Statistics"
...
A "code-only" version of this tutorial is available in the `basic-statistic-tutorial.R` script. A completed version of tutorial is available in `basic-statistics-completed.html`.
...
Fill in the chunk below to use `head` to look at the first six rows of the penguins data.

```{r, eval = FALSE}
head()
```

We can also use `skim` to look at the distribution of data and identify missing data.
...
Complete the code chunk below so that we plotting `body_mass_g` on the y-axis.
...
ggplot(penguins, aes(x = body_mass_g, fill = species,  group = species)) +
  geom_histogram() +
  #facet wrap creates a seperate panel for each species
...
Sepal length and width have a clearer linear relationship.
...
Fill in the code below to create a histogram of `body_mass_g`
...
We can also test for if the data is normal using a Shapiro-Wilk's test. Use `?shapiro.test` to explore what arguments this function takes.

Fill in the code below to do a test on the `body_mass_g` variable from the `penguins` dataset.
...
Fill in the code below to create a violin plot of `body_mass_g` for each `species`.
...
We will use Levene's test because it is generally more robust. The Levene's test is in the `car` package. It uses a formula similar to those used in linear regression. The continuous response variable is to the left of the tilde (~) and the grouping variable is to the right.
...
Before applying a test of correlation, we should first visualize the data using a scatterplot. One variable is plotted on the x-axis and the other on the y-axis. Here we will compare flower sepal length and petal width from the `flowers` dataset. 

Fill in the code chunk below using `geom_point` from `ggplot`.
...
Fill in the chunk below to conduct a Shapiro-Wilk test on  the variable `Sepal.Width`.
...
Both types of correlation test's use the `cor.test` function. You specify which type of test to use via the `method` argument. If you don't specify, it will use a Pearson's test by default. 
...
Is this association positive or negative? How strong is it? Is it statistically significant?
...
The `t.test` function uses a formula format (~). Fill in the code below to conduct a two-sample t-test for the variable `bill_length_mm` across the group `sex`. Ensure that you specify whether the variance is equal or not. (Hint: Use `?t.test` to see what arguments the function requires.)
...
Based on this boxplot, do you think the bill Length differs between Biscoe and Dream islands?

Let's do a Mann-Whitney test to see if there is a difference between the two groups.  Fill in the chunk below with the correct formula to explore `bill_length_mm` across `island`.

```{r, eval = FALSE}
wilcox.test(FORMULA, data = penguins,
...
Based on our data visualization, we suspect that Gentoo penguins are significantly larger than the other two species, so will do a post-hoc comparison to compare Gentoo penguins with Adelie and Chinstrap penguins and calculate and adjusted p-value.
...
Fill in the code chunk below using `geom_histogram` and `facet_wrap` to view the distribution of `flipper_length_mm` by `island`.
...
```{r, eval = FALSE}
kruskal.test(FORMULA, data = penguins)
```
